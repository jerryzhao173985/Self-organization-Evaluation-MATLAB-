...
function derivitive_cal_plot(T1, index)
    for i = index:0.1:( (index+1)-0.1 )
        ...
        % adding title to tje distribution plot
        ii = (i-1)*10
        minute = 5 + floor(ii/60);  %Here use floor, notice rounding: >.5 then add 1
        second = mod(ii,60);
        titlename = append('At time: ', int2str(minute),' min ',int2str(second), ' sec');
        %title({'Output distribution and peak'; titlename,{'Color','blue'} });
        title('Output distribution and peak')
        subtitle(titlename,'Color','b')
